function plot_NAO_property( obj, property_name, time_vec, MOVIE_FLAG )
...
for time_ind = 1 : obj.num_samples,
    % Plot state
    if ~isempty(obj.data(time_ind).NAO.(property_name)),
        h = plot(time_vec(time_ind), obj.data(time_ind).NAO.(property_name), ...
            'marker', obj.data(time_ind).NAO.marker, 'color', obj.data(time_ind).NAO.colour);
        
        if obj.data(time_ind).NAO.isEstimate && est_legend_flag == 0,
            legend_handles = [legend_handles, h];
            legend_str{end+1} = 'Estimate';
            est_legend_flag = 1;
        end
        if obj.data(time_ind).NAO.isTruth && truth_legend_flag == 0,
            legend_handles = [legend_handles, h];
            legend_str{end+1} = 'Truth';
            truth_legend_flag = 1;
        end;
        if obj.data(time_ind).NAO.isObservation && doa_legend_flag == 0,
            legend_handles = [legend_handles, h];
